...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  pMsg->msg [i+1]= buf[i];   
  osal_msg_send( App_TaskID, (byte *)pMsg ); //在第7步里的语句MT_UartRegisterTaskID(task_id)中//App_TaskID被赋了应用层的任务号SampleApp_TaskID，所以消息是发往应用层的
...
void MT_UartProcessZToolData_OLD ( uint8 port, uint8 event )
{
  uint8  ch;
